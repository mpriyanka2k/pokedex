...
class PokedexDetailNotifire extends StateNotifier<PokedexDetailState>{
  ...
  PokedexDetailNotifire(this.usecase) : super(PokedexDetailState()){
    // getPokedexDetail();
  }
  ...
  Future<void> getPokedexDetail(String id) async{
    ...
    try{
      ...
      if(result is ApiSuccess){
        ...
        state = PokedexDetailState(loading: false,data: result.data);
      }
      else if(result is ApiError){
        state = PokedexDetailState(error: result.message);
      }

    }
    catch(e){
      state = PokedexDetailState(loading: false,error: e.toString());
    }
  }
}